...
class TestModelConfigMeasurement(unittest.TestCase):
    def setUp(self):
        ...
        self.throughput_record = PerfThroughput(1000)
        self.latency_record = PerfLatencyP99(20)

        self.perf_metrics = {
            PerfThroughput.tag: self.throughput_record,
            PerfLatencyP99.tag: self.latency_record,
        }

        self.mcmA = ModelConfigMeasurement(self.perf_metrics)
    ...
    def test_basic_accessor_methods(self):
        ...
        self.assertEqual(self.mcmA.get_perf_metrics(), self.perf_metrics)
        self.assertEqual(
            self.mcmA.get_perf_metric(PerfLatencyP99.tag), self.latency_record
        )
        self.assertEqual(
            self.mcmA.get_perf_metric_value(PerfThroughput.tag, return_value=-1),
            self.throughput_record.value(),
        )
        self.assertEqual(
            self.mcmA.get_perf_metric_value("metric not in dict", return_value=-1), -1
        )
