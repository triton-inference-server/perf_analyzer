...
class TestInputRetrieverFactory:
    ...
    def test_create_file_retrieverg(self):
        config = InputsConfig(input_type=PromptSource.FILE, input_filename=Path("input_data.jsonl"))
        with patch("genai_perf.inputs.retrievers.file_input_retriever.FileInputRetriever.__init__", return_value=None) as mock_init:
            retriever = InputRetrieverFactory.create(config)
            mock_init.assert_called_once_with(config)
            assert isinstance(retriever, FileInputRetriever), "Should return a FileInputRetriever"
    ...
    def test_create_synthetic_retriever(self):
        ...
        with patch("genai_perf.inputs.retrievers.synthetic_data_retriever.SyntheticDataRetriever.__init__", return_value=None) as mock_init:
            retriever = InputRetrieverFactory.create(config)
            mock_init.assert_called_once_with(config)
