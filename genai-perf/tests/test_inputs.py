...
class TestInputs:
    ...
    @patch.object(Tokenizer, 'encode', return_value=[1243, 1881, 697])  # Mock Tokenizer encode method
    @patch("genai_perf.tokenizer.get_tokenizer")  # Mock the get_tokenizer function
    def test_data_retrieval_and_conversion(self, mock_get_tokenizer, mock_encode, mock_open_fn, mock_converter_factory, mock_retriever_factory):
        ...
        mock_open_fn.assert_called_once_with(str(inputs.config.output_dir / "inputs.json"), "w")
        mock_open_fn().write.assert_called_once_with(json.dumps(expected_output, indent=2))

    @patch("genai_perf.inputs.inputs.InputRetrieverFactory.create")
    @patch("genai_perf.inputs.inputs.OutputFormatConverterFactory.create")
    @patch("builtins.open", new_callable=mock_open)
    @patch("genai_perf.tokenizer.get_tokenizer")
    def test_write_json_to_file(self, mock_get_tokenizer, mock_open_fn, mock_converter_factory, mock_retriever_factory):
        mock_tokenizer = MagicMock(spec=Tokenizer)
        mock_get_tokenizer.return_value = mock_tokenizer
        ...
        mock_retriever_factory.return_value.retrieve_data.assert_called_once()
        mock_converter_factory.return_value.convert.assert_called_once_with(generic_dataset, inputs.config)
